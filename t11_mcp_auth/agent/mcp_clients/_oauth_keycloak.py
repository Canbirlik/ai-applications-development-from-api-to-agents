...
class OAuthTokenManager:
    ...
    async def authenticate(self) -> None:
        ...
        if not ready_event.is_set():
            raise TimeoutError("Timed out waiting for Keycloak authentication callback")

        if code_holder.get("error"):
            raise RuntimeError(f"Keycloak authentication failed: {code_holder['error']}")
        ...
        if not auth_code:
            raise RuntimeError("No authorization code received from Keycloak")
